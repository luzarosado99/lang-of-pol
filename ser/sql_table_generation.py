"""
This was the first attempt at creating an algorithm that systematically parsed the pickle files to add specified data to a .sql file, containing CREATE statements.
These created .sql files were to be imported into postgres and manually added.
"""
...
zone = sys.argv[1]
sql_path = f"./{zone}.sql"
core_path = "/project2/graziul/LoP/audio/"
...
def insertion_from_properties(file_name, file_location, zone, prop_dict):
    ...
    entries = sql_insertion_input(file_name, file_location, file_length_seconds, day, month, time, year, zone)
    return f"({entries})"
...
def vad_insertion_from_properties(file_name, file_location, zone, vad_prop_dict):
    ...
    vad_entries = sql_insertion_input(file_name, file_location, nonsilent_minutes, nonsilent_slices) # TODO add db & pydub to this statement
...
def daily_insertion_from_properties(directory_location, zone, dictionary, metadata_endswith):
    ...
    daily_entries = sql_insertion_input(directory_name, directory_location, day_length_minutes, files_total_silence, complete_data, has_silent_files)
