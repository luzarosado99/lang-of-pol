#1 parse & store audio file

#2 pre-process the actual audio
# extract and store features
#---> dynamically do pca??

# extract metadata & store in postgres table
import parselmouth
...
# create utils file that we can import
# transcripts to mp3 files

class Zone(Date):
    def __init__(self, zone):
        self.zone = zone
        audio_path = find("Zone" + str(zone))
        self.files = glob.glob(audio_path + '/2*.mp3')
        self.num_processors = 28
        self.audio_dict = {}


    def load_audio(self, files, num_processors = self.num_processors):
        files.sort()
        p = Pool(processes=num_processors)
        output = p.map(SingleAudio.get_mp3_file, files)
        p.close()
...
class Date(SingleAudio):
    def __init__(self):
        self.date = date
        self.files = os.listdir(os.path.join(base_path, zone)) # TODO replace base_path w something meaningful that finds the actual base_path
        self.audio_dict = {}
...
class SingleAudio():
    def __init__(self, file, file_location):
        ...
        self.np_array = self.pydub_to_np(self)
    ...
    def extract_parselmouth(self, feature):
...
class ParselPraat(Extraction):
    def __init__(self, ):
        super()

    def sound_to_feature_array(self, sound = self.sound, feature):
